...
import numpy as np
from transformers import BartTokenizer, Seq2SeqTrainingArguments, BartConfig, EarlyStoppingCallback
import time
import json
import logging
import os
...
require_version("apex==0.1", "To fix: 1) git clone https://github.com/NVIDIA/apex    2) cd apex    3) python setup.py install")
...
class One2MultiSeqDataset(torch.utils.data.Dataset):
...
def fetch_datasets(training_args=None):
    ...
    train_dataset = One2MultiSeqDataset(train_data)
    val_dataset = One2MultiSeqDataset(valid_data)
    test_dataset = One2MultiSeqDataset(test_data)
...
def check_valid_keyphrases(str_list):
    ...
    for i, word_list in enumerate(str_list):
        ...
        for w in word_list:
            if w == UNK_WORD or w == ',' or w == '.' and len(word_list) == 1:
                keep_flag = False
                print(word_list)
...
def compute_metrics(eval_preds, num_return_sequences=1, topk_list=[5,'M'], present_tags =['all','present','absent'],src_lines=None, meng_rui_precision = False):
    ...
    src_text,decoded_preds, decoded_labels = postprocess_text(src_lines,decoded_preds, decoded_labels, '<sep>')
    ...
    for topk in topk_list:
        for present_tag in present_tags:
            ...
            result['macro_avg_precision_%s_%s' % (topk,present_tag)]=names['macro_avg_precision_%s_%s' % (topk,present_tag)]
            result['macro_avg_recall_%s_%s' % (topk,present_tag)]=names['macro_avg_recall_%s_%s' % (topk,present_tag)]
...
def main(seed, data_name, singe_data_args, test_dataset_name,do_train):
    training_args = Seq2SeqTrainingArguments_(
        ...
        encoder_input_length=singe_data_args['encoder_input_length'],
        decoder_input_length=singe_data_args['decoder_input_length'],
        ...
        data_type='One2MultiSeq',  # select the training dataset, 'One2MultiSeq' or 'One2Seq'
